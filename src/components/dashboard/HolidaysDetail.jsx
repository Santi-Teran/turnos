...
const HolidaysDetail = ({ holidays, handleUpdate, handleDelete }) => {
  ...
  const [editedHoliday, setEditedHoliday] = useState({ id: '', name: '', date: ''});
            ...
            <tr key={holiday.id} className='border-b border-gray-200 hover:bg-gray-100'>
              ...
              <td className="py-3 px-6">
                {editingId === holiday.id ? (
                  ...
                  <div className="flex items-center justify-center space-x-2 text-lg">
                    <button onClick={() => startEditing(holiday)} className="text-blue-500">
                      <FaEdit />
                    </button>
                    <button onClick={() => handleDelete(holiday.id)} className="text-red-500">
                      <FaTrash />
                    </button>
                  </div>
                )}
